...
func (c ItemController) Route(auth middlewaresecurity.AuthorizationHandler) http.Handler {
	rt := chi.NewRouter()
	resource := model.ResourceItem
	rt.Get("/", auth.Register(resource, model.ActionRead, http.HandlerFunc(c.loadAll)))
	rt.Get("/{itemID}", auth.Register(resource, model.ActionRead, http.HandlerFunc(c.load)))
	rt.Put("/", auth.Register(resource, model.ActionWrite, http.HandlerFunc(c.save)))
	rt.Delete("/{itemID}", auth.Register(resource, model.ActionWrite, http.HandlerFunc(c.delete)))
	return rt
}
